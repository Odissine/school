...
def nouvelle_annee():
    ...
    for i in range(1,6):
        ...
        for user in users:
            user.groups.add(groupe_to_change)
...
def download_score():
    ...
    # Préparer un dictionnaire pour stocker les données
    data = {}

    for group in groups:
        users = User.objects.filter(groups=group)
        quiz_instances = QuizInstance.objects.filter(player__in=users)

        # Initialiser une liste pour stocker les résultats du groupe
        group_data = []

        for user in users:
            for quiz_instance in quiz_instances.filter(player=user):
                quiz = Quiz.objects.get(id=quiz_instance.quiz_id)
                group_data.append({
                    'User': user.username,
                    'Quiz': quiz.nom,
        ...
        # Ajouter les données du groupe au dictionnaire
