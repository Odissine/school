...
def download_players(request, quiz_id=None):
    ...
    for classe in classes:
        ...
        try:
            ...
            data[quiz.name] = [{'Eleve':instance.player, 'Score':instance.score} for instance in instances]
            quiz_dic[classe.name] = data
            print(quiz_dic)
        except:
            print("ko")
    ...
    with BytesIO() as b:
        ...
        try:
            ...
            quiz_id = "all"
            quiz_name = "all"
        filename = 'ExportResultats_'+ str(quiz_id) + '_' + str(quiz_name) + '.xlsx'
    ...
    with BytesIO() as buffer:
        with pd.ExcelWriter(buffer) as writer:
            for group_name, group_data in data.items():
                ...
                # Écrire le DataFrame dans l'onglet du groupe
                df.to_excel(writer, sheet_name=group_name, index=False)
        response = HttpResponse(
            buffer.getvalue(),
            content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
        response['Content-Disposition'] = f'attachment; filename=ExportScoreQuiz.xlsx'
